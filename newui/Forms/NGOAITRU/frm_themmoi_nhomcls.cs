...
namespace VNS.HIS.UI.NGOAITRU
{
    public partial class frm_themmoi_nhomcls : Form
    {
        ...
        private DataTable m_dtIdChitiet = new DataTable();
        ...
        public frm_themmoi_nhomcls(string nhomchidinh, byte chidinhgoi, byte chidinhchiphithem)
        {
            InitializeComponent();
            InitEvents();
        ...
        private void frm_themmoi_nhomcls_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (grdAssignDetail.RowCount > 0)
            {
                if (!isSaved)
                {
                    if (m_dtIdChitiet.Select("NoSave=1").Length > 0)
                    {
                        if (
            ...
            try
            {
                ...
                if (sqlQuery.GetRecordCount() <= 0)
                {
                            ...
                            "Thông báo", true))
                    {
                        ...
                        txtId.Text = "(Tự sinh)";
                        //txtManhom.Text = THU_VIEN_CHUNG.SinhMaChidinhCLS();
                    }
                }
                m_blnCancel = true;
                ModifyCommand();
        ...
        void resetNewItem()
        {
            if (m_dtIdChitiet == null || !m_dtIdChitiet.Columns.Contains("isnew")) return;
            foreach (DataRow dr in m_dtIdChitiet.Rows)
                dr["isnew"] = 0;
            m_dtIdChitiet.AcceptChanges();
        }
        void SetSaveStatus()
        ...
        private void AddDetail()
        {
            try
            {
                ...
                foreach (GridEXRow gridExRow in ArrCheckList)
                {
                    ...
                    EnumerableRowCollection<DataRow> query = from loz in m_dtIdChitiet.AsEnumerable().Cast<DataRow>()
                                                             where
                                                                 Utility.Int32Dbnull(
                    ...
                    if (query.Count() <= 0)
                    {
                        DataRow newDr = m_dtIdChitiet.NewRow();
        ...
        private void cmdInPhieuCLS_Click(object sender, EventArgs e)
        {
            ...
            {
                //string mayin = "";
                //int v_AssignId = Utility.Int32Dbnull(grdAssignDetail.GetValue(DmucNhomcanlamsangChitiet.Columns.IdChidinh), -1);
                //string v_AssignCode = Utility.sDbnull(grdAssignDetail.GetValue(DmucNhomcanlamsang.Columns.MaChidinh), -1);
        ...
        private void LayThongTin_Chitiet_CLS()
        {
            try
            {
                m_dtIdChitiet = CHIDINH_CANLAMSANG.DmucLaychitietNhomchidinhCls(Utility.Int32Dbnull(txtId.Text, -1));
                Utility.SetDataSourceForDataGridEx(grdAssignDetail, m_dtIdChitiet, false, true, "1=1",
        ...
        private void InitData()
        {
            try
            {
                ...
                m_dtServiceDetail = CHIDINH_CANLAMSANG.DmucLaydanhmucclsTaonhomchidinh( nhomchidinh);
                if (!m_dtServiceDetail.Columns.Contains(DmucNhomcanlamsangChitiet.Columns.SoLuong))
                    m_dtServiceDetail.Columns.Add(DmucNhomcanlamsangChitiet.Columns.SoLuong, typeof(int));
