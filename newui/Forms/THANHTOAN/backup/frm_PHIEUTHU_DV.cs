...
{
    public partial class frm_PHIEUTHU_DV : Form
    {
        ...
        private void ChonCauHinhIn()
        {
            try
            {
                if (System.IO.File.Exists(PathKhoGiayA5))
                {
                    string[] khoigay = System.IO.File.ReadAllLines(PathKhoGiayA5);
                    if (khoigay.Count() > 0)
                    {
                        radA5.Checked = Utility.sDbnull(khoigay[0]) == "A5";
                        radA4.Checked = Utility.sDbnull(khoigay[0]) == "A4";
                    }
                }
                else
                {
                    if(globalVariables.MA_KHOA_THIEN=="KKB") radA4.Checked = true;
                    if(globalVariables.MA_KHOA_THIEN=="KYC") radA5.Checked = true;
                    System.IO.File.WriteAllText(PathKhoGiayA5, radA5.Checked ? "A5" : "A4");
                }
                b_hasloadData = true;
            }
        ...
        public frm_PHIEUTHU_DV()
        {
            InitializeComponent();
            InitEvents();
            THU_VIEN_CHUNG.GetTieuDeBaoCao(this.Name, txtTieuDe.Text);
            ...
            switch (THU_VIEN_CHUNG.LayMaDviLamViec())
            {
                case "KYDONG":
                    ...
                    cmdINPHIEU_CHOBENHNHAN.Visible = true;
                   // cmdRefundPayment.Visible = true;
                    cmdINPHIEU_CHOBENHNHAN.Text = "In cho bệnh nhân";
        ...
        void InitEvents()
        {
            this.Load += new System.EventHandler(this.frm_YHHQ_PhieuThu_Load);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frm_YHHQ_PhieuThu_KeyDown);
            this.cmdExit.Click += new System.EventHandler(this.cmdExit_Click);
            this.cmdInPhieuXN.Click += new System.EventHandler(this.cmdInPhieuXN_Click);
            this.txtSO_TIEN.TextChanged += new System.EventHandler(this.txtSO_TIEN_TextChanged);
            this.cmdConfigPrint.LinkClicked += new System.Windows.Forms.LinkLabelLinkClickedEventHandler(this.cmdConfigPrint_LinkClicked);
            ...
            this.cmdINPHIEU_CHOBENHNHAN.Click += new System.EventHandler(this.cmdPrintPhuThu_Click);
            radA5.CheckedChanged += new EventHandler(radA5_CheckedChanged);
            radA4.CheckedChanged += new EventHandler(radA4_CheckedChanged);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void radA4_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                WriteCauHinhIn();
        ...
        private void setProperties()
        {
            foreach (Control control in grpTongTien.Controls)
            {
                if (control is Janus.Windows.GridEX.EditControls.EditBox)
                {
                    ...
                    txtTien.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far;
                }
            }
        }
        private void txtTieuDe_LostFocus(object sender, EventArgs eventArgs)
        {
            THU_VIEN_CHUNG.UpdateTieuDe(this.Name, txtTieuDe.Text);
        }
        #endregion
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_YHHQ_PhieuThu_Load(object sender, EventArgs e)
        {
            if (status == 0)
            {
              ...
              //  cmdRefundPayment.Visible = THU_VIEN_CHUNG.IsDichVu(ObjectTypeId);
            }
            else
            {
        ...
        private void YHHQ_PrintPhieuThu(string sTitleReport)
        {
            ...
            //if (status != 0)
            //crpt.SetParameterValue("NguoiChi", THU_VIEN_CHUNG.GetStaffByUserName(dataRow["NGUOI_NOP"].ToString()));
        ...
        private void cmdInPhieu_Click(object sender, EventArgs e)
        {
            ...
            actionResult = _THANHTOAN.UpdateDataPhieuThu(CreatePhieuThu(), CreatePaymentDetail());
            switch (actionResult)
            {
                case ActionResult.Success:
                    ...
                    switch (sAccountName)
                    {
                        ...
                        case "DETMAY":
                            ...
                            break;
                        default:
                            if (THU_VIEN_CHUNG.Laygiatrithamsohethong("PHIEU_NGOAITRU_DV",false)=="HONGPHUC")
                            {
        ...
        private void YHHQ_InPhieuDV(bool IsTongHop)
        {
            ...
            m_dtReportPhieuThu = _THANHTOAN.GetDataInphieuDichvu(objPayment);
        ...
        private void YHHQ_InPhieuBH(bool IsTongHop)
        {
            TPayment objPayment = TPayment.FetchByID(Utility.Int32Dbnull(txtPayment_ID.Text, -1));
            if (IsTongHop) objPayment.PaymentId = -1;
            m_dtReportPhieuThu = _THANHTOAN.GetDataInphieuBH(objPayment,true);
        ...
        private void YHHQ_InPhieuBH_IN_BN(bool IsTongHop)
        {
            TPayment objPayment = TPayment.FetchByID(Utility.Int32Dbnull(txtPayment_ID.Text, -1));
            if (IsTongHop) objPayment.PaymentId = -1;
            m_dtReportPhieuThu = _THANHTOAN.INPHIEUBH_CHOBENHNHAN(objPayment);
        ...
        private void KyDong_InPhieuDV(bool IsTongHop)
        {
            ...
            _THANHTOAN.XuLyThongTinPhieu_DichVu(ref m_dtReportPhieuThu);
            KYDONG_PrintDichVu(txtTieuDe.Text);
        }
        ...
        private void KYDONG_PrintDichVu(string sTitleReport)
        {
            ...
            try
            {
                string PHIEU_NGOAITRU_DV = Utility.sDbnull(THU_VIEN_CHUNG.Laygiatrithamsohethong("PHIEU_NGOAITRU_DV", "KYDONG",false));

                switch (PHIEU_NGOAITRU_DV)
                {
                    case "KYDONG":
                        new INPHIEU_THANHTOAN_NGOAITRU().KYDONG_INPHIEU_DICHVU(m_dtReportPhieuThu,dtNgayIn.Value, sTitleReport,radA4.Checked?"A4":"A5");
                        break;
                    case "LAOKHOA":
                        new INPHIEU_THANHTOAN_NGOAITRU().KYDONG_INPHIEU_DICHVU(m_dtReportPhieuThu, dtNgayIn.Value, sTitleReport, radA4.Checked ? "A4" : "A5");
                        break;
                    case "HONGPHUC":
                        //new INPHIEU_NGOAITRU().HONGPHUC_INPHIEU_DICHVU(m_dtReportPhieuThu, sTitleReport);
                        break;
                    default:
                        new INPHIEU_THANHTOAN_NGOAITRU().KYDONG_INPHIEU_DICHVU(m_dtReportPhieuThu, dtNgayIn.Value, sTitleReport, radA4.Checked ? "A4" : "A5");
                        break;
        ...
        private void KyDong_InPhieuBH_DungTuyen(bool IsTongHop)
        {
            ...
            m_dtReportPhieuThu = _THANHTOAN.KYDONG_GetDataInphieuBH(objPayment,false);
                ///load thông tin của việc lấy dữ liệu vào datatable trong phiếu thanh toán
        ...
        private void KyDong_InPhieuBH_BENHNHAN(bool IsTongHop)
        {
            ...
            if (radInTatCa.Checked) objPayment.PaymentId = -1;
            m_dtReportPhieuThu =
               _THANHTOAN.KydongInphieuBaohiemChoBenhnhan(objPayment);
        ...
        private void KyDong_InPhieuBH_TraiTuyen(bool IsTongHop)
        {
            TPayment objPayment = TPayment.FetchByID(Utility.Int32Dbnull(txtPayment_ID.Text, -1));
            if (IsTongHop) objPayment.PaymentId = -1;
            m_dtReportPhieuThu = _THANHTOAN.KYDONG_GetDataInphieuBH_TraiTuyen(objPayment);
            ///load thông tin của việc lấy dữ liệu vào datatable trong phiếu thanh toán
        ...
        private void KyDong_InPhieuBH_ChiDinhTraiTuyen()
        {
            ...
            //if (IsTongHop) objPayment.PaymentId = -1;
            m_dtReportPhieuThu =
                _THANHTOAN.KydongInPhieubaohiemTraituyen(objPayment);
            ...
            // crpt.SetParameterValue("CurrentDate", Utility.FormatDateTime(THU_VIEN_CHUNG.GetSysDateTime()));
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void cmdPrintPhuThu_Click(object sender, EventArgs e)
        {
            LObjectType objectType = LObjectType.FetchByID(v_ObjectType_Id);
            switch (THU_VIEN_CHUNG.LayMaDviLamViec())
            {
        ...
        private void DETMAY_InphieuBH(string sTitleReport,bool isTongHop)
        {
            ...
            if (isTongHop) objPayment.PaymentId = -1;
            m_dtReportPhieuThu =
                _THANHTOAN.DetmayPrintAllExtendExamPaymentDetail(objPayment);
            if (m_dtReportPhieuThu.Rows.Count <= 0)
            {
        ...
        private void DETMAY_InphieuBH_PHUTHU(string sTitleReport)
        {
           ...
           // if (isTongHop) objPayment.PaymentId = -1;
            m_dtReportPhieuThu =
                _THANHTOAN.DetmayInphieuBhPhuthu(objPayment);
            if (m_dtReportPhieuThu.Rows.Count <= 0)
            {
                Utility.ShowMsg("Không có dữ liệu tìm thấy,Bạn kiểm tra lại xem đã thanh toán chưa ", "Thông báo");
                return;
        ...
        private void DETMAY_InPhieuDichVuHoacBaohiem_NoiTru()
        {
            ...
            switch (objectType.ObjectTypeType)
            {
                ...
                case 1:
                    ...
                    break;
                case 0:
                    new VietBaIT.HISLink.UI.YHHQ_Reports.Class.DETMAY_InPhieuNoiTru().DETMAY_InphieuBH_NOITRU(
        ...
        {
            m_dtPaymentDetail =
                _THANHTOAN.Laychitietthanhtoan(Utility.Int32Dbnull(txtPayment_ID.Text, -1));
